...
def check_certificate_basic(cert, check_skid=True):
    """
    Some basic checks on certificate
    :param cert: X509Certificate
    :param check_skid: whether to flag a CA certificate that is missing the Subject Key ID extension
    """
    ...
    if check_skid and is_ca_certificate(x509_cert):
...
def check_file_system_certificate(file, check_skid=True):
    """
    ...
    check_certificate_basic(pem_cert, check_skid=check_skid)
...
def check_certificate_status():
    ...
    # The Auto Deploy CA certificate is self-signed and has historically not included the
    # Subject Key ID extension. Since it isn't used to build a certificate chain, its absence
    # here is benign, so this check is skipped as it was in legacy versions of vCert.
    check_file_system_certificate(RBD_CERT_FILE_PATH, check_skid=False)
...
def check_vcenter_extension_thumbprints():
    ...
    for extension, (thumbprint, db_cert_pem) in extension_thumbprints.items():
        ...
        if get_vc_version() < VcVersion.V9:
            ...
            # Legacy extensions carried over from an 8.x upgrade only have a thumbprint
            # in vpx_ext, not a full PEM certificate. vpxd tolerates this by falling back
            # to thumbprint-only authentication, so only require the PEM to match when one
            # is actually present in the database.
            if thumbprint == expected_thumbprint and (not db_cert_pem or db_cert_pem == expected_cert_pem):
                print_task_status('MATCHES')
            else:
                print_task_status_warning('MISMATCH')
                check_result = False
